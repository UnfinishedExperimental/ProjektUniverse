...
import universetournament.client.logic.entities.controller.ShipResetController;
...
import universetournament.shared.logic.entities.controller.PlayerAttribute;
 ...
 * der Zeit- und Punktebegrenzung, sowie die allgemeine Spielerverwaltung
 * und Score-Verwaltung für Kills und Deaths bietet.
 * Übeschreibt handleEvent und refresh Methoden des Oberklasse für besonderes
	...
	extends EntityController<UTGameModeData>
{
	...
	/**
	 * Punktzahl, die ein Spieler für einen Tod bekommt.
	 * Wahrscheinlich ist hier ein negativer Wert gewünscht.
	 */
	 ...
	 * Standardmäßig erhält jeder Spieler für einen Kill einen Punkt und
	 * verliert einen für jeden Tod.
	 * Wirft eine NullpointerException, wenn die übergebenen Spieldaten null
	 * sind.
	 * 
	...
	/**
	 * Setzt die verbleibende Zeit in Sekunden.
	 * Werden negative Werte übergeben wird diese auf 0 gesetzt.
	 * 
	...
	/**
	 * Gibt dem Spieler die Punkte für einen Kill und erhöht seinen
	 * Kill-Zähler um 1.
	 * Ignoriert Aufrufe mit unbekannten IDs.
	 * 
	...
	/**
	 * Verschickt an die Clients einen Event, welcher mitteilt, dass das
	 * Spiel vorbei ist und gegebenenfalls, wie es ausgegangen ist.
	 * Sollte von Unterklassen für verschiedenes Verhalten angepasst werden.
	 */
	 ...
	 * setzen.
	 * Wird bei der Erstellung des Schiffs oder beim "Respawnen" aufgerufen.
	 * Sollte von den Unterklassen enstprechendend überschrieben werden.
	 * In der Standard-Implementierung werden die Schiffe zufällig auf dem
	 * Spielfeld positioniert.
	 * Wird das erste Schiff gesetzt werden ebenfalls Dekorationen gesetzt.
	 * Sendet entsprechende Override-Events.
	 * Ignoriert null.
	 * 
	...
	public void placeShip(ShipEntity ship)
	{
		if(ship != null)
		{
			...
			Vec3 pos = new Vec3((float)Math.sin(angle),  0.f,
					(float)Math.cos(angle));
			pos.normalize().mult((float)(Math.random() * mapSize), pos);
			Vec3 vel = new Vec3();
	...
	/**
	 * Kickt den Spieler mit dem angegebenen Index in der internen Liste,
	 * sofern vorhanden.
	 * 
	 * @param index
	 */
	...
	protected void spawnDecorations()
	{
		if(start)
		{
			...
			while(decorationNum-- > 0)
			{
				...
				pos = new Vec3((float)Math.sin(angle),  0.f,
						(float)Math.cos(angle));
				pos.normalize().mult((float)(Math.random() * mapSize), pos);
