...
void main(){
    ...
    vec4 baseCol = texture( diffuse_sampler, texCoord );
    vec4 noiseCol = texture( noise_sampler, texcoord + vec2( noiseOffsetX, noiseOffsetY ) * ( 1.0 + noiseOffsetX + noiseOffsetY ) ) * 0.5 + vec4( 0.25 );
